                ...
                <div class="form-card-body">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="description" name="description"
                            class="form-textarea">{{ old('description', $projet->description) }}</textarea>
                    </div>
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <select id="statut" name="statut" class="form-select" required>
                                @foreach(['actif' => 'Actif', 'archive' => 'Archivé'] as $val => $label)
                                    <option value="{{ $val }}" {{ old('statut', $projet->statut) === $val ? 'selected' : '' }}>
                                        {{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                            ...
                            @foreach($clients as $client)
                                <div class="checkbox-item">
                                    <input type="checkbox" id="cl_{{ $client->id }}" name="clients[]" value="{{ $client->id }}"
                                        {{ $projet->clients->contains($client->id) ? 'checked' : '' }} />
                            ...
                            @foreach($collaborateurs as $collab)
                                <div class="checkbox-item">
                                    <input type="checkbox" id="co_{{ $collab->id }}" name="collaborateurs[]"
                                        value="{{ $collab->id }}" {{ $projet->collaborateurs->contains($collab->id) ? 'checked' : '' }} />
                            ...
                            @foreach($contrats as $contrat)
                                <div class="checkbox-item">
                                    <input type="checkbox" id="ct_{{ $contrat->id }}" name="contrats[]" value="{{ $contrat->id }}"
                                        {{ $projet->contrats->contains($contrat->id) ? 'checked' : '' }} />
